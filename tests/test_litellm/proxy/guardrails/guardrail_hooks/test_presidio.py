...
async def test_mask_streaming_preserves_stream_on_check_pii_error():
    """
    A transient Presidio failure on one chunk must not truncate the response.
    The failed chunk is redacted (fail closed, never leaking the PII it could
    not mask) while later chunks and the finish chunk still reach the client.
    """
    guardrail = _OPTIONAL_PresidioPIIMasking(mock_testing=True, apply_to_output=True)
